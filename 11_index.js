...
for (user of users) {
    //for (i in user.favoritesSounds) {
        //console.log(Object.getOwnPropertyNames(user.favoritesSounds));
        sounds =  [...sounds,...Object.getOwnPropertyNames(user.favoritesSounds)];
        //console.log(sounds);
        //elementCount ++;
    //};
};
...
//console.log(sounds);
const nonRepeatedSounds = [...new Set(sounds)];
let result = [];

for (elem in nonRepeatedSounds) {
    let count = sounds.filter(val => val === nonRepeatedSounds[elem]).length;
    //console.log(sounds.filter(val => val === nonRepeatedSounds[elem]));
    //repetitionNumber = [...repetitionNumber,...count];
    let soundRepetition = {sound: nonRepeatedSounds[elem],repetitions: count};
    //console.log(soundRepetition)
    result.push(soundRepetition);
    //console.log(result)
};
